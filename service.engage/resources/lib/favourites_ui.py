...
def _settings_dialog_visible():
    """
    <summary>
    Whether one of Kodi's add-on settings dialogs is on screen right now.
    </summary>
    <returns>True when either window name in SETTINGS_WINDOW_NAMES is active.</returns>
    <remarks>
    Kodi has used both names for the dialog across releases, so both are checked.
    </remarks>
    """
...
def handle_action(action, *args):
    """
    <summary>
    Dispatch an action name from script.py or a RunScript call to the matching UI function.
    </summary>
    <param name="action">One of manage_slots, open_settings, open_favs, add_fav, remove_fav or pick_fav.</param>
    <param name="args">Further RunScript arguments; only pick_fav uses one, the slot number.</param>
    <remarks>
    An unknown name is logged as a warning and shown as a toast rather than raised.
    slot_manager is imported inside the function because it imports the favourites
    helpers from this module, and a top level import would be circular.
    </remarks>
    """
